...
export default function Vista() {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/herramientas`)
      .then(response => {
  ...
  useEffect(() => {
    if (categoriaSeleccionada) {
      axios.get(`${API_URL}/subcategorias`)
        .then(response => {
          const filteredSubcategorias = response.data.filter(sub => sub.categoria_id === parseInt(categoriaSeleccionada));
  ...
  useEffect(() => {
    if (subcategoriaSeleccionada) {
      axios.get(`${API_URL}/tipos-herramienta`)
        .then(response => {
          const filteredTipos = response.data.filter(tipo => tipo.subcategoria_id === parseInt(subcategoriaSeleccionada));
...
const herramientasFiltradas = herramientas.filter(herramienta => {
  return (
    (!categoriaSeleccionada || herramienta.categoria_id === parseInt(categoriaSeleccionada)) &&
    (!subcategoriaSeleccionada || herramienta.subcategoria_id === parseInt(subcategoriaSeleccionada)) &&
    (!tipoSeleccionado || herramienta.tipo_id === parseInt(tipoSeleccionado)) &&
    (!busqueda || 
      (herramienta.tipo_nombre && herramienta.tipo_nombre.toLowerCase().includes(busqueda.toLowerCase())) || // Filtrar por tipo de herramienta
...
const consumiblesFiltrados = consumibles.filter(consumible => {
  return (
    (!categoriaSeleccionada || consumible.categoria_id === parseInt(categoriaSeleccionada)) &&
    (!subcategoriaSeleccionada || consumible.subcategoria_id === parseInt(subcategoriaSeleccionada)) &&
    (!busqueda || 
      (consumible.tipo_nombre && consumible.tipo_nombre.toLowerCase().includes(busqueda.toLowerCase())) || // Filtrar por tipo de consumible
            ...
            {herramientasFiltradas.map(herramienta => (
              <Card className="mb-3" key={herramienta.id}>
                <Card.Header as="h5" className="d-flex justify-content-between align-items-center">
                  <span>{herramienta.tipo_nombre} ID: #{herramienta.id}</span>
          ...
          <Tab.Pane eventKey="consumables">
            ...
            {consumiblesFiltrados.map(consumible => (
              <Card className="mb-3" key={consumible.id}>
                <Card.Header as="h5" className="d-flex justify-content-between align-items-center">
                  <span>{consumible.nombre} ID: #{consumible.id}</span>
